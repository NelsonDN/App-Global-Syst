...
@section('title')
Modifier une plaque - Données géographiques 
@endsection
        ...
        <div class="col-lg-4">
            <div class="page-header-breadcrumb">
                <ul class="breadcrumb-title">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('plaques.index') }}">Liste des plaques</a>
                    </li>
                    <li class="breadcrumb-item"><a href="{{ route('plaques.create') }}">Ajouter</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="col-sm-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5>Modifier une plaque</h5>
                <span>Bien vouloir<code>remplir tous</code>les<code>champs</code>!</span>

            </div>
            <div class="card-block">
                <form id="main" method="POST" action="{{ route('plaques.update', $plaque->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">Type de plaque</label>
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="name" value="{{ $plaque->name }}">
                            @error('name')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="courant" value="{{ $plaque->courant }}">
                            @error('courant')
                                <li style="color:red;">{{ $message }}</li>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="tension" value="{{ $plaque->tension }}">
                            @error('tension')
                                <li style="color:red;">{{ $message }}</li>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" name="duree_de_vie" value="{{ $plaque->duree_de_vie }}">
                            @error('duree_de_vie')
                                <li style="color:red;">{{ $message }}</li>
                            @enderror
                        </div>
                    ...
                    <table id="basic-btn" class="table table-striped table-bordered nowrap">
                        ...
                        <tbody>
                            @foreach($plaques as $plaque)
                                <tr>
                                    <td>{{ $plaque->name }}</td>
                                    <td>{{ $plaque->courant }}</td>
                                    <td>{{ $plaque->tension }}</td>
                                    <td>{{ $plaque->duree_de_vie }}</td>
                                    <td text-align="align-items-center" >
                                        <a href="{{ route('plaques.edit', $plaque->id )}}">
                                            <button class="btn btn-success mr-2" title="Modifier"><i class="fa fa-pencil"></i></button>
                                        </a>
                                        <a href="{{ route('plaques.destroy', $plaque->id )}}">
